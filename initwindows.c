...
InitFileTypeWindow(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_ExtData[1] = AllocPooled(pool, sizeof(struct MinList))) {
		NewList((struct List *)wd->wd_ExtData[1]);
		foreach(&iotypes, io) {
			if (io->io_Flags & (wd->wd_ExtData[0] ? IOF_READABLE : IOF_WRITEABLE)
				&& (iol = AllocPooled(pool, sizeof(struct IOTypeLink)))) {
				iol->iol_Node.ln_Name = io->io_Node.ln_Name;
				iol->iol_Link = io;
				NewList((struct List *)&iol->iol_Description);

				foreach(&io->io_Description, ln)
					WordWrapText((struct List *)&iol->iol_Description, ln->ln_Name, scr->Width / 3 - 40);

				AddTail(wd->wd_ExtData[1], iol);
			}
		}
	}
}
...
InitDocumentWindow(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_ExtData[3] = AllocPooled(pool,sizeof(struct List))) {
		NewList((struct List *)wd->wd_ExtData[3]);

		foreach(&events,ln) {
			if (cln = AllocPooled(pool,sizeof(struct Node))) {
				cln->ln_Name = ln->ln_Name;
				cln->ln_Type = mp->mp_Events[i].ev_Flags & EVF_ACTIVE ? TRUE : FALSE;
				AddTail(wd->wd_ExtData[3],cln);
			}
			i++;
		}
	}
	...
	if (wd->wd_ExtData[1] = AllocPooled(pool,sizeof(struct List))) {
		...
		for (page = (APTR)((struct Page *)wd->wd_Data)->pg_Mappe->mp_Pages.mlh_Head;page->pg_Node.ln_Succ;page = (APTR)page->pg_Node.ln_Succ) {
			if (wp = AllocPooled(pool, sizeof(struct wdtPrinter))) {
				...
				AddTail((struct List *)wd->wd_ExtData[1],wp);
			}
		}
	}
...
{
	if (wd->wd_ExtData[2] = AllocPooled(pool,sizeof(struct List)))
	{
		struct Prefs *pr = GetLocalPrefs(wd->wd_Data);

		NewList(wd->wd_ExtData[2]);
...
initPrefIcon(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_ExtData[2] = AllocPooled(pool,sizeof(struct MinList)))
	{
		NewList(wd->wd_ExtData[2]);
		foreach(&pr->pr_IconObjs,io)
		{
			if (sio = CopyIconObj(io))
				AddTail(wd->wd_ExtData[2],(struct Node *)sio);
		}
	}
	if (wd->wd_ExtData[4] = AllocPooled(pool,sizeof(struct MinList)))
	{
		NewList(wd->wd_ExtData[4]);
		MakePrefIconAppCmds(pr,wd->wd_ExtData[4]);
	}
	if (wd->wd_ExtData[3] = AllocPooled(pool,sizeof(struct ImageNode)))
...
initPrefColors(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_Data = AllocPooled(pool,sizeof(struct List)))
	{
		NewList(wd->wd_Data);
...
{
	if (wd->wd_Data = AllocPooled(pool, sizeof(struct PrefFile)))
	{
...
{
	if (wd->wd_ExtData[0] = AllocPooled(pool, sizeof(struct PrefTable)))
...
InitFormatPrefsWindow(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_ExtData[5] = AllocPooled(pool,sizeof(struct List)))
	{
		NewList((struct List *)wd->wd_ExtData[5]);
		for(sfv = (APTR)pr->pr_Formats.mlh_Head;sfv->fv_Node.ln_Succ;sfv = (APTR)sfv->fv_Node.ln_Succ)
		{
			if (fv = CopyFormat(sfv))
				AddTail((struct List *)wd->wd_ExtData[5],fv);
		}
	}
}
...
InitObjectWindow(reg (a0) struct winData *wd)
{
	...
	if (wd->u.object.wd_Gadgets = AllocPooled(pool, sizeof(struct MinList)))
		NewList(wd->u.object.wd_Gadgets);
}
...
InitDiagramWindow(reg (a0) struct winData *wd)
{
	...
	if (wd->u.diagram.wd_Gadgets = AllocPooled(pool, sizeof(struct MinList)))
		NewList(wd->u.diagram.wd_Gadgets);
}
...
void AddNotesFromPage(struct Page *page,struct List *list)
{
	...
	foreach(&page->pg_Table,tf)
	{
		...
		if ((s = tf->tf_Note) && !IsInNotes(list,s))
		{
			...
			if (wn = AllocPooled(pool,sizeof(struct wdtNote)))
			{
				char t[NOTELEN];
				int  i = 0;
...
initNotes(reg (a0) struct winData *wd)
{
	...
	if (wd->wd_ExtData[0] = AllocPooled(pool, sizeof(struct List)))
	{
		NewList((struct List *)wd->wd_ExtData[0]);
...
InitBorderWindow(reg (a0) struct winData *wd)
{
	...
	for(;i < 4;i++)
	{
		if (wb = wd->wd_ExtData[i] = AllocPooled(pool,sizeof(struct wdtBorder)))
		{
		...
		for(sdb = (APTR)mp->mp_Databases.mlh_Head;sdb->db_Node.ln_Succ;sdb = (APTR)sdb->db_Node.ln_Succ)
		{
			if (db = AllocPooled(pool,sizeof(struct Database)))
			{
				*db = *sdb;
				...
				AddTail((struct List *)wd->wd_ExtData[2],db);
			}
		}
	}
}
...
CurrentInitDB(struct MinList *mlh,struct Database *db)
{
	...
	{
		if (db)
			cdb = (struct Database *)FindName(mlh,db->db_Node.ln_Name);
		else
	...
	if (wd->wd_ExtData[1] = AllocPooled(pool,sizeof(struct MinList)))
	{
		...
		for(db = (APTR)rxpage->pg_Mappe->mp_Databases.mlh_Head;db->db_Node.ln_Succ;db = (APTR)db->db_Node.ln_Succ)
		{
			if (ma = AllocPooled(pool,sizeof(struct Mask)))
			{
				...
				if (sma = (struct Mask *)FindName(&rxpage->pg_Mappe->mp_Masks,db->db_Node.ln_Name))
				{
	...
	if (wd->wd_ExtData[1] = AllocPooled(pool,sizeof(struct MinList)))
	{
		...
		foreach(&rxpage->pg_Mappe->mp_Databases,db)
		{
			if (cdb = AllocPooled(pool,sizeof(struct Database)))
			{
				CopyMem(db,cdb,sizeof(struct Database));
				cdb->db_Node.ln_Name = AllocString(db->db_Node.ln_Name);
				NewList(&cdb->db_Filters);
				foreach(&db->db_Filters,fi)
				{
					if (cfi = AllocPooled(pool,sizeof(struct Filter)))
					{
