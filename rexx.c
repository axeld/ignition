...
rxBlock(long *opts)
{
	...
	if ((opts[1] && !opts[4]) || AslRequestTags(fileReq,ASLFR_Window,		scr->FirstWindow,
...
rxLoadPicture(long *opts)
{
	...
	if (opts[1])
	{
		if (AslRequestTags(fileReq,ASLFR_Window,		scr->FirstWindow,
	...
	if ((mp = NewProject()) != 0) {
		...
		if (opts[3]) {
			if ((type = (APTR)FindTag(&iotypes, (STRPTR)opts[3])) == NULL) {
				...
				foreach(&iotypes, io) {
					if (io->io_Short && !stricmp(io->io_Short, (STRPTR)opts[3])) {
						type = io;
						break;
					}
				}
...
initRexx(void)
{
	...
	AddIntCmd(rxNop,		   ICF_NONE,  "SESSIONLIST NUM/N");
