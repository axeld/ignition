...
makeLayerClip(struct Layer *l,long x,long y,long w,long h)
{
	...
	return InstallClipRegion(l, region);
}
...
RefreshAppWindow(struct Window *win, struct winData *wd)
{
	...
	switch (wd->wd_Type)
	{
		...
		case WDT_PREFFILE:
			...
			DrawGroupBorder(rp,GetString(&gLocaleInfo, MSG_SAVE_BORDER),lborder,win->BorderTop+fontheight*4+30,win->Width-lborder-rborder,fontheight*4+25);
			break;
		case WDT_PREFTABLE:
