...
CreateBorderGadgets(reg (a0) struct winData *wd)
{
	...
	gWidth = TLn(GetString(&gLocaleInfo, MSG_OK_GAD)) + TLn(GetString(&gLocaleInfo, MSG_FRAME_BLOCK_GAD)) + TLn(GetString(&gLocaleInfo, MSG_CANCEL_GAD)) + 94;
...
CreateCellGadgets(reg (a0) struct winData *wd)
{
    ...
    ngad.ng_GadgetText = GetString(&gLocaleInfo, MSG_ASSIGN_GAD);
    ngad.ng_GadgetID++;                      // 34
    gad = CreateGadget(BUTTON_KIND,gad,&ngad,GT_Underscore,'_',TAG_END);
...
CreateFormelGadgets(reg (a0) struct winData *wd)
{
    ...
    gWidth = 4*TLn(GetString(&gLocaleInfo, MSG_MATHEMATICS_GAD))+206+lborder+rborder;
    ...
    ngad.ng_LeftEdge += 6+ngad.ng_Width;
    ngad.ng_Width += 2*TLn(GetString(&gLocaleInfo, MSG_MATHEMATICS_GAD));
...
CreatePageSetupGadgets(reg (a0) struct winData *wd)
{
    ...
    ngad.ng_LeftEdge = lborder+16+TLn(GetString(&gLocaleInfo, MSG_BOTTOM_MARGIN_LABEL));
    ngad.ng_TopEdge += 2*fontheight+15;
    ngad.ng_Width = gWidth-rborder-8-ngad.ng_LeftEdge;
    ngad.ng_GadgetID++;                     // 5
...
CreateDocumentGadgets(reg (a0) struct winData *wd)
{
	...
	gWidth = 104 + TLn(GetString(&gLocaleInfo, MSG_PROTECT_DOCUMENT_LABEL))
