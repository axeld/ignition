...
{
	DrawBevelBox(win->RPort, win->BorderLeft + 4, wd->wd_FormY + 4, wd_PosWidth, fontheight + 4, GT_VisualInfo,
		vi, GTBB_Recessed, TRUE, GTBB_FrameType, BBFT_BUTTON);
    ...
    while (!ende)
    {
		...
		while (msg = (struct IntuiMessage *)GTD_GetIMsg(iport))
        {
            ...
            if (imsg.Class == IDCMP_MENUPICK && imsg.Code != MENUNULL)
                HandleMenu();
            else if (imsg.Class == IDCMP_MENUHELP || imsg.Class == IDCMP_RAWKEY && imsg.Code == 95)
