...
void ASM CreatePrefMenuGads(REG(a0, struct winData *wd))
{
	...
	ngad.ng_Width = TLn(GetString(&gLocaleInfo, MSG_CANCEL_GAD))+40;
	ngad.ng_GadgetText = GetString(&gLocaleInfo, MSG_OK_GAD);
	ngad.ng_GadgetID++;					 // 21
	gad = CreateGadget(BUTTON_KIND,gad,&ngad,TAG_END);
...
CreateFilePrefsGadgets(REG(a0, struct winData *wd))
{
	...
	gHeight = barheight + fontheight * 9 + 65 + bborder;
...
CreatePrefIconGads(REG(a0, struct winData *wd))
{
	...
	ngad.ng_LeftEdge += ngad.ng_Width;
	ngad.ng_Width = TLn(GetString(&gLocaleInfo, MSG_DELETE_GAD))+50;
	ngad.ng_GadgetText = GetString(&gLocaleInfo, MSG_DELETE_GAD);
	ngad.ng_GadgetID++;					 // 4
	gad = CreateGadget(BUTTON_KIND,gad,&ngad,GT_Underscore,'_',TAG_END);
...
CreatePrefColorsGads(REG(a0, struct winData *wd))
{
	...
	gad = wd->wd_ExtData[0] = NewObj(wd,WOT_GADGET,NULL,"gradientslider.gadget",GA_Top,	   barheight+fontheight+5,
																				...
																				TAG_END);
	gad = wd->wd_ExtData[1] = NewObj(wd,WOT_GADGET,NULL,"colorwheel.gadget",GA_Top,		barheight+fontheight+5,
																			GA_Left,	   w2,
																			GA_Width,	  w,
																			GA_Height,	 h,
																			WHEEL_Screen,  scr,
